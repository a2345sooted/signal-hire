    ...
    try:
        ...
        if job_id:
            ...
            if job_data:
                ...
                import asyncio
                logger.info(f"[RESUME_PROCESSOR] [{clean_id_str}] Triggering Analyzer Agent for job_id: {job_id}, resume_id: {resume_id}")
                asyncio.create_task(
                    run_analyzer_agent(
                        job_id=job_id,
                        resume_id=resume_id,
        ...
        if thread_id_str != NO_THREAD_ID:
            pass
        logger.info(f"[RESUME_PROCESSOR] [{clean_id_str}] Resume saved with ID: {resume_id}")
