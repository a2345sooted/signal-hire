...
from fastapi import Form, Depends, UploadFile, File, BackgroundTasks, Response
from fastapi.responses import StreamingResponse
from sqlalchemy.ext.asyncio import AsyncSession

from ..agents.jd_processor.run import run_jd_agent
from ..agents.resume_processor.run import run_resume_agent, cancel_resume_agent
...
async def save_jd(
    ...
    """
    logger.info(f"Received save JD request for department: {department}")
    ...
    background_tasks.add_task(run_jd_agent, raw_text=job_description, job_id=job_id, department=department)
...
async def upload_resume(
    ...
    """
    logger.info(f"Received resume upload for job_id: {job_id}, filename: {resume.filename}")
...
async def get_resume_analysis(
    ...
    analysis = await repo.get_analysis_by_resume(resume_id)
    if not analysis:
...
async def get_analysis(
    ...
    """
    Retrieve a specific analysis by ID, including resume_id and job markdown.
    """
    ...
    resume_repo = ResumeRepository(db)
    resume = await resume_repo.get_resume_by_id(uuid.UUID(analysis_data["resume_id"]))
    if not resume:
        return {"success": False, "message": "Resume not found for analysis"}
    
    job_repo = JobRepository(db)
    job = await job_repo.get_job_by_id(uuid.UUID(resume["job_id"]))
    if not job:
         return {"success": False, "message": "Job not found for resume"}
    
    return {
        "analysis": analysis_data["content"],
        "resume_id": analysis_data["resume_id"],
        "resume_name": resume.get("filename") or resume.get("original_filename") or "Unknown",
        "resume_email": (resume.get("structured_data") or {}).get("contact", {}).get("email") or "No email",
        "resume_structured_data": resume.get("structured_data") or {},
        "job_title": job.get("title") or "Untitled Job",
...
async def delete_job_endpoint(job_id: uuid.UUID, db: AsyncSession = Depends(get_db)):
    """
